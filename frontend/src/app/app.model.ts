...
export function useAppModel(): AppViewProps {
  ...
  const indexMutation = useMutation({ mutationFn: saveIndex, onSuccess: () => queryClient.invalidateQueries({ queryKey: ['indices'] }) });
  const deleteIndexMutation = useMutation({ mutationFn: removeIndex, onSuccess: () => queryClient.invalidateQueries({ queryKey: ['indices'] }) });
  const comparisonMutation = useMutation({ mutationFn: ({ next, nextCorrelations }: { next: Comparison; nextCorrelations: Record<string, number> }) => saveComparison(next, nextCorrelations), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['comparison'] }) });
  ...
  return {
    ...
    onFundsChange: (funds) => { const industry = funds.find(fund => fund.origin === 'industria'); if (industry) fundMutation.mutate(industry); },
    onIndicesChange: (indices) => { const index = indices.at(-1); if (index) indexMutation.mutate(index); },
    ...
    onComparisonChange: (next) => { setComparison(next); queryClient.setQueryData(['comparison'], { comparison: next, correlations: currentCorrelations }); comparisonMutation.mutate({ next, nextCorrelations: currentCorrelations }); },
    onCorrelationsChange: (next) => { setCorrelations(next); queryClient.setQueryData(['comparison'], { comparison: currentComparison, correlations: next }); comparisonMutation.mutate({ next: currentComparison, nextCorrelations: next }); },
  };
}
