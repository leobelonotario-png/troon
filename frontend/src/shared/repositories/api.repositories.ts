...
type ApiIndex = { id: string; name: string; ret: number | null; vol: number | null; updatedAt: string; color: string | null; dashed: boolean };
type ApiComparison = { refId: string | null; selected: string[]; title: string; source: string; period: string; correlations: Record<string, number> };
...
const typeFromApi: Record<string, FundType> = { LIQUID: 'liquido', ILLIQUID: 'iliquido', LISTED: 'listado' };
const typeToApi: Record<FundType, string> = { liquido: 'LIQUID', iliquido: 'ILLIQUID', listado: 'LISTED' };
...
export function toFund(value: ApiFund, position = 0): Fund {
  ...
  return {
    id: value.id, origin: value.origin === 'INDUSTRY' ? 'industria' : 'aprovado', name: value.name,
    cnpj: string('fundRegistrationNumber'), shore: value.domicile === 'OFFSHORE' ? 'Offshore' : 'Onshore',
    type: typeFromApi[string('fundType')] ?? 'liquido', status: statusFromApi[string('status')] ?? 'Aberto',
    classe: string('assetClass'), sub: string('subtype'), bench: string('benchmark'), liq: string('liquidity'),
    trib: string('taxation'), gestora: string('managerName'), data: string('data'), prev: value.recommended === true,
...
function toApiFund(draft: FundDraft) {
  return {
    name: draft.name, origin: draft.origin === 'industria' ? 'INDUSTRY' : 'APPROVED',
    fundRegistrationNumber: draft.cnpj || null, domicile: draft.shore === 'Offshore' ? 'OFFSHORE' : 'ONSHORE',
    fundType: draft.classe && draft.sub ? typeToApi[draft.type] : null, status: draft.status === 'Fechado' ? 'CLOSED' : 'OPEN',
    assetClass: draft.classe || null, subtype: draft.sub || null, benchmark: draft.bench || null,
...
export const updateFundMetrics = (updates: Array<Pick<Fund, 'id' | 'ret' | 'vol' | 'updatedAt'>>) => request<void>('/admin/funds/metrics', { method: 'POST', body: JSON.stringify({ updates }) });
export const importFundsCsv = (file: File) => {
  const data = new FormData();
  data.append('file', file);
  return fetch(`${apiUrl}/funds/import`, { method: 'POST', body: data }).then(async response => {
    if (!response.ok) throw new Error(((await response.json().catch(() => null)) as { error?: string } | null)?.error ?? 'Não foi possível importar o CSV.');
  });
};

export async function getTaxonomy(): Promise<Taxonomy> {
  const data = await request<Record<string, Array<{ assetClass: string; subtypes: string[] }>>>('/fund-taxonomy');
  return {
...
function toIndex(value: ApiIndex): Index { return { ...value, updatedAt: value.updatedAt.slice(0, 10), color: value.color ?? '#5B9BD5' }; }
export const listIndices = async () => (await request<ApiIndex[]>('/indices')).map(toIndex);
export const saveIndex = async (index: Index) => toIndex(await request<ApiIndex>(index.id.startsWith('idx-') ? '/indices' : `/indices/${index.id}`, { method: index.id.startsWith('idx-') ? 'POST' : 'PATCH', body: JSON.stringify({ name: index.name, ret: index.ret, vol: index.vol, color: index.color, dashed: index.dashed }) }));
export const removeIndex = (id: string) => request<void>(`/indices/${id}`, { method: 'DELETE' });
export const getComparison = async (): Promise<{ comparison: Comparison; correlations: Record<string, number> }> => { const value = await request<ApiComparison>('/comparison'); return { comparison: { refId: value.refId, selected: value.selected, titulo: value.title, fonte: value.source, periodo: value.period }, correlations: value.correlations }; };
export const saveComparison = (comparison: Comparison, correlations: Record<string, number>) => request<ApiComparison>('/comparison', { method: 'PUT', body: JSON.stringify({ refId: comparison.refId, selected: comparison.selected, title: comparison.titulo, source: comparison.fonte, period: comparison.periodo, correlations }) });
