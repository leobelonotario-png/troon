...
export function useQuickUpdateModalModel({ onClose }: QuickUpdateModalProps): QuickUpdateViewProps {
  ...
  const importMutation = useMutation({
    mutationFn: importFundsCsv,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['funds'] });
      setFeedback('CSV importado com sucesso.');
    },
  });
  ...
  useEffect(() => {
    if (!rows.length && fundsQuery.data) {
      setRows(
        fundsQuery.data.reduce<QuickRow[]>((result, fund) => {
          if (fund.validated && fund.origin === 'aprovado')
            result.push({
              id: fund.id,
              name: fund.name,
              cnpj: fund.cnpj,
  ...
  const onApplyPaste = () => {
    const sources = (fundsQuery.data ?? []).filter(
      (fund) => fund.validated && fund.origin === 'aprovado',
    );
    let matched = 0;
  ...
  const onSave = () => {
    if (rows.some((row) => parse(row.ret) === null || parse(row.vol) === null)) {
  ...
  return {
    ...
    isCsvSubmitting: importMutation.isPending,
    ...
    onImportCsv: () => {
      ...
      else setFeedback('Selecione um arquivo CSV.');
    },
    onSave,
    onClose,
  };
